...
app.post('/book/:roomId', (req, res) => {
    ...
    res.redirect(`/personal-info/${newBooking.id}`);
});

// Personal info route
app.get('/personal-info/:bookingId', (req, res) => {
    if (!req.session.user) return res.redirect('/login');
    const bookingId = req.params.bookingId;
    res.render('personal_info', { bookingId, error: null });
});

app.post('/personal-info/:bookingId', (req, res) => {
    if (!req.session.user) return res.redirect('/login');
    const bookingId = req.params.bookingId;
    const { specialRequest, numberOfGuests } = req.body;

    if (!specialRequest || !numberOfGuests) {
        return res.render('personal_info', { bookingId, error: 'Please fill in all fields.' });
    }

    // Update the booking with personal info
